...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        {
            await ShowFirstLaunchDialogIfNeeded();
        };
    }
    ...
    // ===== 首次启动提示（社区版） =====
    private async Task ShowFirstLaunchDialogIfNeeded()
    {
        var config = LauncherConfig.Load();
        if (config.IsFirstLaunchShown)
            return;

        var dialog = new ContentDialog
        {
            Title = "⚠️ 欢迎使用社区版",
            Content = "ObsMCLauncher-CE 是社区维护版本，非官方发布。\n\n如果您是误下载，请访问官方版：\nhttps://github.com/mcobs/ObsMCLauncher",
            PrimaryButtonText = "我知道了",
            SecondaryButtonText = "跳转官方版",
        ...
        config.IsFirstLaunchShown = true;
        config.Save();
    }
